...
// Bilinear Interpolation to sample pixel colors smoothly
...
// Forward Mapping Function: (x,y) -> (x', y')
...
Image warpImage(const Image& src) {
    ...
    for (int r = 0; r < dst.height; ++r) {
        for (int c = 0; c < dst.width; ++c) {
            ...
            // If mapping returns coordinates outside original image, set to black
...
// 2. Function to reverse the spatial warp
Image reverseWarpImage(const Image& warped) {
    ...
    for (int r = 0; r < recovered.height; ++r) {
        for (int c = 0; c < recovered.width; ++c) {
            ...
            // Use forward map to find where this pixel went in the warped image
