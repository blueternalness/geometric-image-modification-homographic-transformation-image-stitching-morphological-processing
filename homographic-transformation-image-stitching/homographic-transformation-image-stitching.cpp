...
Mat getHomography(const Mat& img1, const Mat& img2) {
    ...
    for (int i = 0; i < 4 && i < pts1.size(); ++i) {
        cout << round(pts1[i].x) << ", " << round(pts1[i].y) 
             << "->" 
    ...
    if (good_matches.size() >= 4) {
        ...
        Mat img_matches;
        drawMatches(img1, kp1, img2, kp2, matches_to_draw, img_matches, Scalar::all(-1), Scalar::all(-1), vector<char>(), DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
...
int main() {
    ...
    for (int y = 0; y < panorama.rows; ++y) {
        for (int x = 0; x < panorama.cols; ++x) {
            ...
            if (pM != Vec3b(0,0,0)) {
                panorama.at<Vec3b>(y, x) = pM;
            } else if (pL != Vec3b(0,0,0)) {
                panorama.at<Vec3b>(y, x) = pL;
            } else if (pR != Vec3b(0,0,0)) {
                panorama.at<Vec3b>(y, x) = pR;
            }
        }
    }
